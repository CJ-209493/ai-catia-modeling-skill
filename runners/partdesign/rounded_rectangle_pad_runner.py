"""Runner for recipe partdesign.rounded_rectangle_pad."""
...
RECIPE_ID = "partdesign.rounded_rectangle_pad"
EXPECTED_NATIVE_FEATURE = "Pad"
REFERENCE_PATTERN_ID = "partdesign.tangent_arc_closed_profile"


def normalize_params(params):
    return {
        "part_number": params.get("part_number", "Rounded_Rectangle_Pad"),
        "width": float(params.get("width", 100.0)),
        "height": float(params.get("height", 50.0)),
        "corner_radius": float(params.get("corner_radius", 10.0)),
...
def build_rounded_rectangle_pad(params, output_dir, feature_id="rounded_rectangle_pad_1", mode="user"):
    ...
    create_rounded_rectangle_pad(
        ...
        body,
        ref_xy,
        sketch_name="RoundedRectangle_Profile",
        feature_name="RoundedRectangle_Pad",
        width=params["width"],
    ...
    report = make_report(
        ...
        notes="Native CATIA Pad created from a closed tangent line/arc rounded-rectangle sketch; Part.Update passed.",
        extra={"params": params, "part_number_assignment": part_number_result},
    )
    write_report(output_dir, "rounded_rectangle_pad_report.json", report)
    return report
...
def main():
    ...
    print(json.dumps(build_rounded_rectangle_pad(feature["params"], output_dir, feature["id"], mode), indent=2))
