...
def test_mirror_and_shell_are_user_mode_runners():
    expected = {
        "partdesign.native_mirror_plane": (
            "Mirror",
            "runners/partdesign/native_mirror_plane_runner.py",
            "partdesign.mirror_plane_ref",
        ),
        "partdesign.native_shell_selected_face": (
            "Shell",
            "runners/partdesign/native_shell_selected_face_runner.py",
...
def test_transform_shell_reference_manifest_documents_required_references():
    manifest = load_yaml("manifests/reference_manifest.yaml")
    patterns = {item["id"]: item for item in manifest["reference_patterns"]}

    expected = {
        "partdesign.mirror_plane_ref": ["mirror"],
        "partdesign.shell_selected_face_ref": ["shell"],
    }
...
def test_promoted_transform_shell_cards_record_native_feature_contracts():
    expected = {
        "recipes/partdesign/native_mirror_plane.yaml": ("Mirror", "partdesign.mirror_plane_ref"),
        "recipes/partdesign/native_shell_selected_face.yaml": ("Shell", "partdesign.shell_selected_face_ref"),
    }
    ...
    for recipe_path, (native_feature, reference_pattern_id) in expected.items():
        ...
        assert any(native_feature in step for step in card["verified_pattern"])


def test_promoted_transform_shell_runners_emit_schema_reports():
    cases = [
        (
            "runners/partdesign/native_mirror_plane_runner.py",
            "partdesign.native_mirror_plane",
...
def test_capability_manifest_marks_mirror_and_shell_as_executable():
    capability = load_yaml("manifests/capability_manifest.yaml")
    partdesign = capability["capabilities"]["partdesign"]

    assert "partdesign.native_mirror_plane" in partdesign["mirror"]["executable_recipe_ids"]
    assert partdesign["mirror"]["user_mode"] == "executable_runner_available"
    assert "partdesign.native_shell_selected_face" in partdesign["shell"]["executable_recipe_ids"]
    assert partdesign["shell"]["user_mode"] == "executable_runner_available"
